...
import { syncYargitay } from "@/lib/sync/syncYargitay";
...
async function syncAllSources(): Promise<SyncResult[]> {
        ...
        if (!existing) {
          ...
          await supabase
            ...
            .update({
              metadata: {
                ...currentMetadata,
                country: currentMetadata.country || "TR",
                updated: new Date().toISOString(),
              },
            })
            .eq("id", existing.id);
  ...
  // 2. Yargıtay
  const yargitayResult = await syncYargitay(supabase);
  results.push(yargitayResult);
